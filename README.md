...
```python
city_keys = [k for k in greenville.keys()]
...
```python
city_values = [v for v in greenville.values()]
...
```python
pyeongchang_values = [v for v in cities[11].values()]
...
```python
pyeongchang_keys = [k for k in cities[11].keys()]
